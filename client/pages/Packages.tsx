...
export default function Packages() {
  const location = useLocation() as {
    state?: { transition?: string; category?: string };
  };
  const defaultTab =
    categories.find((c) => c.key === location.state?.category)?.key ||
    "website";
  const transitionClass =
    location.state?.transition === "slide-left"
      ? "animate-slideLeft"
      : "animate-fadeIn";
      ...
      <section className={`container py-12 ${transitionClass}`}>
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            Flexible plans designed for small businesses to scale online.
          </p>
        </div>
              ...
              >
                <div className="grid gap-6 md:grid-cols-3">
                  {tiers.map((t) => (
                    <Card
                      key={`${c.key}-${t.name}`}
                      className="flex flex-col rounded-2xl border shadow-sm bg-white"
                    >
                      <CardHeader>
                  ...
                  {c.key === "branding" && (
                    <Card className="flex flex-col rounded-2xl border shadow-sm bg-white">
                      <CardHeader>
                        <span className="inline-flex w-fit rounded-full bg-secondary px-2 py-1 text-xs text-foreground/70">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Creative Branding for Your Business
                        </p>
                        <p className="text-3xl font-bold text-primary">
                          ₹1500{" "}
                      ...
                      <CardFooter className="mt-auto">
                        <Button
                          className="w-full rounded-full"
                          onClick={() => onGet("branding", "Logo Designing")}
                        >
                          Get Your Logo
                        </Button>
                      </CardFooter>
